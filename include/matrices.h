...
glm::vec4 crossproduct(glm::vec4 u, glm::vec4 v)
{
    ...
    return glm::vec4(u2 * v3 - u3 * v2, // Primeiro coeficiente
                     ...
                     u1 * v2 - u2 * v1, // PREENCHA AQUI o terceiro coeficiente de um produto vetorial, preechido!
...
float dotproduct(glm::vec4 u, glm::vec4 v)
{
    ...
    return u1 * v1 + u2 * v2 + u3 * v3 /* PREENCHA AQUI o que falta para definir o produto escalar, preenchido! */;
}
...
glm::mat4 Matrix_Perspective(float field_of_view, float aspect, float n, float f)
{
    ...
    float b = -t; // preenchido!
    float r = t * aspect;
    float l = -r; // preenchido!
