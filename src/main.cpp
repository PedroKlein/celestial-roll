...
// renderização.
float g_CameraTheta = (float)M_PI;
float g_CameraPhi = -(float)M_PI / 6;
...
int main()
{
    ...
    GLFWwindow *window;
    window = glfwCreateWindow(800, 800, "INF01047 - 00324104 - Pedro Klein", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    glm::vec4 camera_position_c = glm::vec4(0.0f, 3.0f, 3.0f, 1.0f); // Ponto "c", centro da câmera
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (g_SKeyPressed)
            camera_position_c -= cameraSpeed * camera_view_vector;
        if (g_AKeyPressed)
            camera_position_c -= cameraSpeed * camera_right_vector;
        if (g_DKeyPressed)
            camera_position_c += cameraSpeed * camera_right_vector;
